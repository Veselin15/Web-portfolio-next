...
export default function Projects() {
  ...
  return (
        ...
        >
          <AnimatePresence mode="popLayout">
            {filteredProjects.map((project) => (
              ...
              >
                {activeTab === "Software" ? (
    // --- SOFTWARE CARD DESIGN (Links to GitHub/RepoUrl) ---
    <a
    ...
    >
        <div className="group h-full bg-slate-900/50 border border-slate-800 rounded-2xl p-6 flex flex-col hover:border-sky-500/50 hover:bg-slate-900 transition-all duration-300 hover:shadow-2xl hover:shadow-sky-500/10 cursor-pointer">
            <div className="flex justify-between items-start mb-4">
                <div className="p-3 bg-sky-500/10 rounded-lg text-sky-400 group-hover:scale-110 transition-transform">
                    <Layers size={24}/>
                </div>
                <div className="text-slate-500 text-sm font-mono group-hover:text-sky-400 transition-colors flex items-center gap-1">
            ...
            <div className="flex flex-wrap gap-2 mt-auto">
                {project.tech.slice(0, 3).map((t) => (
                    <span key={t} className="text-xs font-mono text-sky-300/80 bg-sky-500/10 px-2 py-1 rounded border border-sky-500/20">
                        {t}
                    </span>
                ))}
            </div>
                  ...
                  <Link href={`/project/${project.id}`} className="block h-full">
                    <div className="group h-full relative overflow-hidden rounded-2xl border border-slate-800 bg-slate-900 hover:border-yellow-500/50 transition-all duration-500">
                      ...
                      <div className="relative h-52 overflow-hidden">
                        ...
                        <div className="absolute top-4 right-4 z-20 bg-yellow-500/10 backdrop-blur-md border border-yellow-500/20 text-yellow-400 text-xs font-bold px-3 py-1 rounded-full">
                          Hardware
                        </div>
                      </div>
